...
角色左右分栏（电商后台 / 运营 / 仓管 / WMS），流程自上而下，横线跨栏交接。不按库存/入库/出库拆图。

可打开同目录页面查看与示例一致的排版：`2026-08-19-mini-wms-swimlane.html`

```mermaid
flowchart LR
  subgraph 电商后台
    direction TB
    S((开始)) --> A1[新建 Product / SKU]
    A1 --> A2[导出订单 Excel]
    A2 --> E((结束))
  end
